...
namespace ber {
    ...
    template <size_t number_static_values, size_t max_values>
    template <typename Writer>
    inline bool encoder<number_static_values, max_values>::encode(Writer& writer) const
    {
        if (_M_parent == -1) {
            for (size_t i = 0; i < _M_used; i++) {
    ...
    template <size_t number_static_values, size_t max_values>
    inline size_t encoder<number_static_values, max_values>::value::length() const
    {
            ...
            if (writer.write(len, lenlen)) {
                ...
                case value::type::BitstringDeepCopy:
                    ...
                    if ((bitlen & 0x07) == 0) {
                        ...
                        if ((!writer.write(&unused, 1)) || (!writer.write(ptr, vlen - 1))) {
                            return false;
                        }
                    } else {
                        ...
                        uint8_t c = static_cast<const uint8_t*>(ptr)[vlen - 2] & (static_cast<uint8_t>(0xff) << unused);
    ...
    encoder<number_static_values, max_values>::get(size_t idx) const {
        return (idx < number_static_values) ? &_M_static_values[idx] : &_M_dynamic_values[idx - number_static_values];
    }
